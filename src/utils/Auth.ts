...
export class Auth {
     ...
     */
    async comparePassword(candidatePassword: string, studentPassword: string): Promise<boolean> {
        return new Promise((resolve, reject) => {
            Bcrypt.compare(candidatePassword, studentPassword, ((err: any, isSame: any) => {
                if (err) {
                    reject(err);
                } else if (!isSame) {
                    resolve(false);
